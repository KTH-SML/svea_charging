...
class stanley_control(rx.Node):
    ...
    goal_yaw_tolerance = rx.Parameter(np.deg2rad(8.0))
    goal_approach_speed = rx.Parameter(0.08)
    goal_approach_gain = rx.Parameter(0.7)
    ...
    def _aruco_goal_cb(self, msg: PoseArray):
        ...
        self.aruco_y = pose.position.z
        self.thetaAruco = pose.orientation.y
        self.get_logger().info(f"x: {self.aruco_x}, y:{self.aruco_y}, theta: {self.thetaAruco}")
    ...
    def loop(self):
        ...
        # keep moving with reduced speed when close to the goal so we can settle without overshooting
        if bool(self.use_adaptive_speed):
            self.controller.target_velocity = self.target_velocity * (self.aruco_distance / 5.0)

        if dist <= self.goal_tolerance:
            approach_scale = np.clip(dist / self.goal_tolerance, 0.0, 1.0) * self.goal_approach_gain
            self.controller.target_velocity = max(
                self.goal_approach_speed,
                self.controller.target_velocity * approach_scale
            )

        steering, velocity = self.controller.compute_control(state)
        self.get_logger().info(f"Steering: {steering}, Velocity: {velocity}")

        if dist <= self.goal_tolerance and abs(self.controller.yaw_error) <= self.goal_yaw_tolerance:
            if abs(self.controller.cross_track_error) <= self.goal_tolerance * 0.5:
